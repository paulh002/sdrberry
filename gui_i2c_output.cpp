...
gui_i2c_output gui_i2coutput;

void gui_i2c_output::save_button_handler_class(lv_event_t *e)
{
	...
	if (code == LV_EVENT_CLICKED)
	{
		...
		Settings_file.save_string("i2c", "output_device", buffer);

		selection = lv_roller_get_selected(addressObj);
		lv_roller_get_selected_str(addressObj, buffer.data(), 79);
		buffer.resize(strlen(buffer.data()));
		Settings_file.save_string("i2c", "output_address", buffer);
		Settings_file.write_settings();
	}
}

void gui_i2c_output::AddressHandler_class(lv_event_t *e)
{
	lv_event_code_t code = lv_event_get_code(e);
	lv_obj_t *obj = lv_event_get_target(e);
}

void gui_i2c_output::deviceDropdownHandler_class(lv_event_t *e)
{
	lv_event_code_t code = lv_event_get_code(e);
	lv_obj_t *obj = lv_event_get_target(e);
...
void gui_i2c_output::init(lv_obj_t *o_tab, lv_coord_t w, lv_coord_t h, lv_group_t *bg)
{
	...
	lv_label_set_text(i2cdevice_label, "I2C output device");
	lv_obj_align_to(i2cdevice_label, deviceDropdown, LV_ALIGN_OUT_TOP_LEFT, 0, -10);

	std::string str = Settings_file.get_string("i2c", "output_device");
	int ii = 0;
	...
	lv_table_set_cell_value(gpiotable, 3, 1, "Antenna 2");
...
void gui_i2c_output::setAdressList(std::string buffer)
{
