...
void FmDecoder_executer::init(double ifrate, double tuner_offset, int pcmrate, bool stereo, double bandwidth_pcm, unsigned int downsample, AudioOutput *audio_output)
{
...
void* rx_fm_thread(void* fm_ptr)
{
	...
	while (!stop_flag.load())
	{
		...
		int sizering = rbuffer.size();
		if(sizering  == 0)
			continue;
		printf("number of samples %d\n", sizering / sizeof(complex<float>));
		
		char *ptr = (char *)rbuffer.read_head();
		
		iqsamples.insert(iqsamples.end(), ptr, (char *)(ptr + sizering));
		rbuffer.consume(sizering);
...
void create_fm_thread(double ifrate, double tuner_offset, int pcmrate, bool stereo, double bandwidth_pcm, unsigned int downsample,  AudioOutput *audio_output)
{
	
	Fm_executer.init(ifrate, tuner_offset, pcmrate, stereo, bandwidth_pcm, downsample, audio_output);
