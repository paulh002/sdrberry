...
static const SDL_Keycode SDL_default_shift_keymap[SDL_NUM_SCANCODES] = {
	/* 0 */ 0,
	/* 1 */ 0,
	/* 2 */ 0,
	/* 3 */ 0,
...
{
	SetKeymap(0, SDL_default_keymap, SDL_default_shift_keymap,  SDL_NUM_SCANCODES);
}

const SDL_Scancode* KeyboardTranslator::SDL_GetScancodeTable(SDL_ScancodeTable table, int *num_entries)
{
	int i;
...
SDL_Scancode KeyboardTranslator::GetScancodeFromTable(SDL_ScancodeTable table, int keycode)
{
...
int KeyboardTranslator::SendKeyboardKey(std::chrono::nanoseconds timestamp, uint8_t state, SDL_Scancode scancode, SDL_KeyboardEvent &event)
{
	return SDL_SendKeyboardKeyInternal(timestamp, KEYBOARD_HARDWARE, state, scancode, SDLK_UNKNOWN, event);
}

int KeyboardTranslator::SDL_SendKeyboardKeyInternal(std::chrono::nanoseconds timestamp, SDL_KeyboardFlags flags, uint8_t state, SDL_Scancode scancode, SDL_Keycode keycode, SDL_KeyboardEvent &event)
{
	SDL_Keymod modifier;
	uint32_t type;
	uint8_t repeat = SDL_FALSE;
	...
	if (keyboard.modstate & SDL_KMOD_CAPS || keyboard.modstate & SDL_KMOD_LSHIFT || keyboard.modstate & SDL_KMOD_RSHIFT)
	{
		event.keysym.key = keycode = keyboard.keymapShift[scancode];
	}
	else
	{
		event.keysym.key = keycode = keyboard.keymap[scancode];
	}
	event.type = type;
	...
	event.keysym.mod = keyboard.modstate;
	return 0;
}

void KeyboardTranslator::SDL_InvalidParamError(std::string param)
{
	printf("Parameter %s is invalid \n", param.c_str());

}

const char *KeyboardTranslator::GetScancodeName(SDL_Scancode scancode)
{
	const char *name;
...
void KeyboardTranslator::SetKeymap(int start, const SDL_Keycode *keys, const SDL_Keycode *shiftkeys, int length)
{
	...
	memcpy(&normalized_keymap[start], keys, sizeof(*keys) * length);
	...
	for (const auto scancode : {SDL_Scancode::SDL_SCANCODE_1, SDL_Scancode::SDL_SCANCODE_9})
	{
		normalized_keymap[scancode] = SDLK_1 + (scancode - SDL_SCANCODE_1);
	}

	// If the mapping didn't really change, we're done here 
	if (!memcmp(&keyboard.keymap[start], &normalized_keymap[start], sizeof(*keys) * length))
	{
		return;
	}

	memcpy(&keyboard.keymap[start], &normalized_keymap[start], sizeof(*keys) * length);
	memcpy(&keyboard.keymapShift[start], &shiftkeys[start], sizeof(*shiftkeys) * length);
}


/*
int KeyboardTranslator::XLookupStringAsUTF8(XKeyEvent *event_struct, char *buffer_return, int bytes_buffer, KeySym *keysym_return, XComposeStatus *status_in_out)
{
