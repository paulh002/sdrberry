...
#define MY_CLASS &lv_cmeter_class

/**********************
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_cmeter_class = {
	.base_class = &lv_obj_class,
	.constructor_cb = cmeter::lv_cmeter_constructor,
	.destructor_cb = cmeter::lv_cmeter_destructor,
	.event_cb = cmeter::lv_cmeter_event,
	.instance_size = sizeof(lv_meter_c)};

cmeter::cmeter(lv_obj_t *parent, lv_coord_t x, lv_coord_t y, lv_coord_t w, lv_coord_t h)
{
	LV_ASSERT_NULL(parent);
	meterObject = lv_cmeter_create(parent);
	lv_obj_set_pos(meterObject, x, y);
	lv_obj_set_size(meterObject, w, h);
	lv_obj_clear_flag(meterObject, LV_OBJ_FLAG_SCROLLABLE);
	...
	lv_obj_add_style(meterObject, &meter_style, 0);
	lv_obj_set_style_pad_hor(meterObject, 0, LV_PART_MAIN);
	lv_obj_set_style_pad_ver(meterObject, 0, LV_PART_MAIN);
	lv_obj_set_style_size(meterObject, w, LV_PART_MAIN);
	lv_obj_set_style_pad_top(meterObject, 20, LV_PART_MAIN);
}

cmeter::~cmeter()
{
	LV_ASSERT_NULL(meterObject);
	lv_obj_del(meterObject);
}

lv_obj_t *cmeter::lv_cmeter_create(lv_obj_t *parent)
{
	LV_LOG_INFO("begin");
...
struct _lv_event_dsc_t *cmeter::lv_obj_add_event_cb(lv_event_cb_t event_cb, lv_event_code_t filter, void *user_data)
{
	return ::lv_obj_add_event_cb(meterObject, event_cb, filter, user_data);
}
...
lv_meter_scale_c *cmeter::lv_meter_add_scale(int16_t r_mod, int16_t c_mod)
{
	LV_ASSERT_OBJ(obj, MY_CLASS);
	lv_meter_c *meter = (lv_meter_c *)meterObject;

	lv_meter_scale_c *scale = (lv_meter_scale_c *)_lv_ll_ins_head(&meter->scale_ll);
	LV_ASSERT_MALLOC(scale);
	lv_memset_00(scale, sizeof(lv_meter_scale_c));
...
void cmeter::lv_meter_set_scale_ticks(lv_meter_scale_c *scale, uint16_t cnt, uint16_t width, uint16_t len,
							  lv_color_t color)
{
	scale->tick_cnt = cnt;
...
void cmeter::lv_meter_set_scale_major_ticks(lv_meter_scale_c *scale, uint16_t nth, uint16_t width,
...
void cmeter::lv_meter_set_scale_range(lv_meter_scale_c *scale, int32_t min, int32_t max, uint32_t angle_range,
							  uint32_t rotation)
{
	scale->min = min;
	scale->max = max;
...
lv_meter_indicator_c *cmeter::lv_meter_add_needle_line(lv_meter_scale_c *scale, uint16_t width,
											   lv_color_t color, int16_t r_mod)
{
	LV_ASSERT_OBJ(obj, MY_CLASS);
	lv_meter_c *meter = (lv_meter_c *)meterObject;
	lv_meter_indicator_c *indic = (lv_meter_indicator_c *)_lv_ll_ins_head(&meter->indicator_ll);
	LV_ASSERT_MALLOC(indic);
	lv_memset_00(indic, sizeof(lv_meter_indicator_c));
	indic->scale = scale;
	indic->opa = LV_OPA_COVER;

	indic->type = LV_CMETER_INDICATOR_TYPE_NEEDLE_LINE;
...
lv_meter_indicator_c *cmeter::lv_meter_add_needle_img(lv_meter_scale_c *scale, const void *src,
											  lv_coord_t pivot_x, lv_coord_t pivot_y)
{
	...
	indic->type = LV_CMETER_INDICATOR_TYPE_NEEDLE_IMG;
...
lv_meter_indicator_c *cmeter::lv_meter_add_arc(lv_meter_scale_c *scale, uint16_t width, lv_color_t color,
									   int16_t r_mod, int16_t c_mod)
{
	...
	indic->type = LV_CMETER_INDICATOR_TYPE_ARC;
	indic->type_data.arc.width = width;
	indic->type_data.arc.color = color;
	indic->type_data.arc.r_mod = r_mod;
	indic->type_data.arc.c_mod = c_mod;

	lv_obj_invalidate(meterObject);
	return indic;
}

lv_meter_indicator_c *cmeter::lv_meter_add_scale_lines(lv_meter_scale_c *scale, lv_color_t color_start,
											   lv_color_t color_end, bool local, int16_t width_mod)
{
	...
	indic->type = LV_CMETER_INDICATOR_TYPE_SCALE_LINES;
...
void cmeter::lv_meter_set_indicator_value(lv_meter_indicator_c *indic, int32_t value)
{
	...
	if (indic->type == LV_CMETER_INDICATOR_TYPE_ARC)
	{
		inv_arc(meterObject, indic, old_start, value);
		inv_arc(meterObject, indic, old_end, value);
	}
	else if (indic->type == LV_CMETER_INDICATOR_TYPE_NEEDLE_IMG || indic->type == LV_CMETER_INDICATOR_TYPE_NEEDLE_LINE)
	{
		inv_line(meterObject, indic, old_start);
		inv_line(meterObject, indic, old_end);
		inv_line(meterObject, indic, value);
	}
	else
	{
		lv_obj_invalidate(meterObject);
...
void cmeter::lv_meter_set_indicator_start_value(lv_meter_indicator_c *indic, int32_t value)
{
	...
	if (indic->type == LV_CMETER_INDICATOR_TYPE_ARC)
	{
		inv_arc(meterObject, indic, old_value, value);
	}
	else if (indic->type == LV_CMETER_INDICATOR_TYPE_NEEDLE_IMG || indic->type == LV_CMETER_INDICATOR_TYPE_NEEDLE_LINE)
	{
		inv_line(meterObject, indic, old_value);
		inv_line(meterObject, indic, value);
	}
	else
	{
...
void cmeter::lv_meter_set_indicator_end_value(lv_meter_indicator_c *indic, int32_t value)
{
...
void cmeter::lv_cmeter_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj)
{
	LV_UNUSED(class_p);
	...
	_lv_ll_init(&meter->scale_ll, sizeof(lv_meter_scale_c));
	_lv_ll_init(&meter->indicator_ll, sizeof(lv_meter_indicator_c));

	LV_TRACE_OBJ_CREATE("finished");
}

void cmeter::lv_cmeter_destructor(const lv_obj_class_t *class_p, lv_obj_t *obj)
{
	LV_UNUSED(class_p);
	LV_ASSERT_OBJ(obj, MY_CLASS);
	lv_meter_c *meter = (lv_meter_c *)obj;
...
void cmeter::lv_cmeter_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
	LV_UNUSED(class_p);
...
void cmeter::draw_arcs(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx, const lv_area_t *scale_area)
{
	lv_meter_c *meter = (lv_meter_c *)obj;
	...
	part_draw_dsc.type = LV_CMETER_DRAW_PART_ARC;

	for (indic = (lv_meter_indicator_c *)_lv_ll_get_tail(&meter->indicator_ll); indic != NULL; indic = (lv_meter_indicator_c *)_lv_ll_get_prev(&meter->indicator_ll, indic))
	{
		if (indic->type != LV_CMETER_INDICATOR_TYPE_ARC)
			continue;

		r_out = lv_area_get_width(scale_area) / 2 + indic->type_data.arc.c_mod;
		scale_center.y = scale_area->y1 + r_out;
		...
		lv_meter_scale_c *scale = indic->scale;
...
void cmeter::draw_ticks_and_labels(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx, const lv_area_t *scale_area)
{
	lv_meter_c *meter = (lv_meter_c *)obj;
	...
	p_center.y = scale_area->y1 + r_edge; //+200;
	//r_edge += 200;
	...
	part_draw_dsc.type = LV_CMETER_DRAW_PART_TICK;
	part_draw_dsc.line_dsc = &line_dsc;

	for (scale = (lv_meter_scale_c *)_lv_ll_get_tail(&meter->scale_ll); scale != NULL; scale = (lv_meter_scale_c *)_lv_ll_get_prev(&meter->scale_ll, scale))
	{
		part_draw_dsc.sub_part_ptr = scale;
		
		r_edge = lv_area_get_width(scale_area) / 2 + scale->c_mod;
		...
		for (i = 0; i < scale->tick_cnt; i++)
		{
			...
			for (indic = (lv_meter_indicator_c *)_lv_ll_get_tail(&meter->indicator_ll); indic != NULL; indic = (lv_meter_indicator_c *)_lv_ll_get_prev(&meter->indicator_ll, indic))
			{
				if (indic->type != LV_CMETER_INDICATOR_TYPE_SCALE_LINES)
					continue;
...
void cmeter::draw_needles(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx, const lv_area_t *scale_area)
{
	lv_meter_c *meter = (lv_meter_c *)obj;
	...
	for (indic = (lv_meter_indicator_c *)_lv_ll_get_tail(&meter->indicator_ll); indic != NULL; indic = (lv_meter_indicator_c *)_lv_ll_get_prev(&meter->indicator_ll, indic))
	{
		...
		if (indic->type == LV_CMETER_INDICATOR_TYPE_NEEDLE_LINE)
		{
			...
			p_end.x = (lv_trigo_cos(angle) * (r_out + scale->c_mod)) / LV_TRIGO_SIN_MAX + scale_center.x;
			//printf("angle %d, p.x %d p.y %d \n", angle, p_end.x, p_end.y);
			...
			part_draw_dsc.type = LV_CMETER_DRAW_PART_NEEDLE_LINE;
			part_draw_dsc.line_dsc = &line_dsc;
			part_draw_dsc.p2 = &p_end;
			scale_center.y = scale_area->y1 + lv_area_get_width(scale_area) / 2 + scale->c_mod;
			part_draw_dsc.p1 = &scale_center;
			lv_event_send(obj, LV_EVENT_DRAW_PART_BEGIN, &part_draw_dsc);
			lv_draw_line(draw_ctx, &line_dsc, part_draw_dsc.p1, &p_end);
			lv_event_send(obj, LV_EVENT_DRAW_PART_END, &part_draw_dsc);
		}
		else if (indic->type == LV_CMETER_INDICATOR_TYPE_NEEDLE_IMG)
		{
			...
			part_draw_dsc.type = LV_CMETER_DRAW_PART_NEEDLE_IMG;
...
void cmeter::inv_arc(lv_obj_t *obj, lv_meter_indicator_c *indic, int32_t old_value, int32_t new_value)
{
...
void cmeter::inv_line(lv_obj_t *obj, lv_meter_indicator_c *indic, int32_t value)
{
	lv_area_t scale_area;
	lv_obj_get_content_coords(obj, &scale_area);
