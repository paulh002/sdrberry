...
void fbdev_flush_rotated(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_p)
{
	if (fbp == NULL ||
		area->x2 < 0 ||
		area->y2 < 0 ||
	...
	/*Truncate the area to the screen*/
	int32_t act_y1 = area->x1 < 0 ? 0 : area->x1;
	int32_t act_x1 = area->y1 < 0 ? 0 : area->y1;
	int32_t act_y2 = area->x2 > (int32_t)vinfo.xres - 1 ? (int32_t)vinfo.xres - 1 : area->x2;
	int32_t act_x2 = area->y2 > (int32_t)vinfo.yres - 1 ? (int32_t)vinfo.yres - 1 : area->y2;
