...
class guiButtonWindows
{
  private:
	...
	void createButtons(std::vector<std::string> buttons, int selected);

	public:
	guiButtonWindows(lv_obj_t *parent, void *thisptr, std::string name, std::vector<std::string> buttons, int selected, lv_event_code_t eventIndex, int width, int height);
	~guiButtonWindows();
