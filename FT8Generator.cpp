...
///
void FT8Generator::gfsk_pulse(int n_spsym, float symbol_bt, float *pulse)
{
...
///
void FT8Generator::synth_gfsk(const uint8_t *symbols, int n_sym, float f0, float symbol_bt, float symbol_period, int signal_rate, float *signal)
{
...
vector<float> FT8Generator::generate(int frequency, encoding code, std::string message)
{
	...
	int rc = pack77(message.c_str(), packed);
	if (rc < 0)
	{
	...
	int sample_rate = audioinput->get_samplerate();
